...
class WXP_Partial_Shipment
{
	...
	function __construct()
	{
		if (!defined('WXP_PARTIAL_SHIP_VER')) {
			define('WXP_PARTIAL_SHIP_VER', 3.3);
		}
	...
	function wxp_order_shipment()
	{
		...
		echo json_encode(array('order_id' => $order_id, 'valid' => $valid, 'products' => $products, 'init' => $init));
		exit();
	}
	...
	{
		global $wpdb;
		$qry = $wpdb->prepare("SELECT id as ship_id FROM " . $wpdb->prefix . "partial_shipment WHERE order_id=%d", $order_id);
	...
	function get_wxp_shipment_data($order_id)
	{
		...
		$shipment_id = $wpdb->get_var($qry);
		if ($shipment_id > 0) {
			$qry = $wpdb->prepare("SELECT * FROM " . $wpdb->prefix . "partial_shipment_items WHERE shipment_id=%d", $shipment_id);
