...
class WXP_Partial_Shipment
{
	...
	function __construct()
	{
		if (!defined('WXP_PARTIAL_SHIP_VER')) {
			define('WXP_PARTIAL_SHIP_VER', 3.5);
		}
	...
	function wxp_admin_head()
	{
		...
		if (isset($screen->id) && in_array($screen->id, array('woocommerce_page_wc-orders', 'shop_order'))) {
			wp_enqueue_style('fancybox', plugins_url('', __FILE__) . '/assets/css/jquery.fancybox.min.css', array(), WXP_PARTIAL_SHIP_VER);
			...
			$js_array = array(
				...
				'wxp_bulk_mark_shipped' => __('Mark as Shipped', 'wc-partial-shipment'),
				'wxp_bulk_mark_not_shipped' => __('Unset Shipped', 'wc-partial-shipment'),
	...
	{
		echo '<th class="wxp-partital-item-head">' . esc_html__('Shipment', 'wc-partial-shipment') . '</th>';
			...
			if (is_a($product, 'WC_Product') && !$product->is_virtual()) {
				...
				if ($item_id && array_key_exists($item_id, $wxp_shipments)) {
					if (isset($shipped['item_qty']) && $shipped['item_qty'] > 0 && $shipped['item_qty'] < $item_data['quantity']) {
						$icon = '<a href="javascript:void(0);" class="wxp-top" title="' . esc_attr($this->wc_partial_labels['partially-shipped']) . ': ' . $shipped['item_qty'] . '/' . $item_data['quantity'] . '"><span class="wxp-partial-shipped wxp-ship-status" title="' . esc_attr($this->wc_partial_labels['partially-shipped']) . '">' . esc_html($this->wc_partial_labels['partially-shipped']) . ' - ' . $shipped['item_qty'] . '</span></a>';
					} elseif (isset($shipped['item_qty']) && $shipped['item_qty'] > 0 && $shipped['item_qty'] == $item_data['quantity']) {
						$icon = '<a href="javascript:void(0);" class="wxp-top" title="' . esc_attr($this->wc_partial_labels['shipped']) . ': ' . $shipped['item_qty'] . '/' . $item_data['quantity'] . '"><span class="wxp-shipped wxp-ship-status" title="' . esc_attr($this->wc_partial_labels['shipped']) . '">' . esc_html($this->wc_partial_labels['shipped']) . ' - ' . $shipped['item_qty'] . '</span></a>';
					} elseif (isset($shipped['item_qty']) && $shipped['item_qty'] < 1) {
						$icon = '<a href="javascript:void(0);" class="wxp-top" title="' . esc_attr($this->wc_partial_labels['not-shipped']) . '"><span class="wxp-not-shipped wxp-ship-status" title="' . esc_attr($this->wc_partial_labels['not-shipped']) . '">' . esc_html($this->wc_partial_labels['not-shipped']) . ' - ' . $item_data['quantity'] . '</span></a>';
				...
				$icon = '<a href="javascript:void(0);" class="wxp-top" title="' . esc_attr($this->wc_partial_labels['shipped']) . ': ' . $item_data['quantity'] . '/' . $item_data['quantity'] . '"><span class="wxp-shipped wxp-ship-status" title="' . esc_attr($this->wc_partial_labels['shipped']) . '">' . esc_html($this->wc_partial_labels['shipped']) . ' - ' . $item_data['quantity'] . '</span></a>';
				echo '<td class="wxp-partital-line-item"><a href="javascript:void(0);"  data-check="' . esc_attr(wp_create_nonce('wxp_check_' . $order_id)) . '"  data-item-id="' . esc_attr($item_id) . '" data-order-id="' . esc_attr($order_id) . '" title="' . esc_html__('Manage Shipment', 'wc-partial-shipment') . '" class="wxp-icons icon-wxp-set-shipping"></a></td>';
				// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- HTML generated internally and sanitized.
				echo '<td class="wxp-partital-item-icon" width="1%">' . $icon . '</td>';
			}
			...
			if ($shipment_id) {
				...
				foreach ($shipped_items as $shipped_item_key => $shipped_item) {
					if ($shipped_item['shipped'] > 0) {
						...
						$qty_shipped = isset($shipped_item['shipped']) && isset($shipped_item['type']) && $shipped_item['type'] === 'shipped' ? intval($shipped_item['shipped']) : 0;
						if (array_key_exists($item_id, $wxp_shipment)) {
							...
							$data = array(
								...
								'item_id' => $item_id,
								'item_qty' => isset($shipped_item['shipped']) ? intval($shipped_item['shipped']) : 0,
							);
							// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching -- Custom plugin table.
		...
		if (is_a($order, 'WC_Order')) {
			...
			if ($show) {
				...
				if (is_a($product, 'WC_Product') && !$product->is_virtual()) {
					if ($item_id && array_key_exists($item_id, $wxp_shipments)) {
						if (isset($shipped['item_qty']) && $shipped['item_qty'] > 0 && $shipped['item_qty'] < $item_data['quantity']) {
							$icon = '<a href="javascript:void(0);" class="wxp-top" title="' . $this->wc_partial_labels['partially-shipped'] . ': ' . $shipped['item_qty'] . '/' . $item_data['quantity'] . '"><span class="wxp-partial-shipped wxp-ship-status" title="' . $this->wc_partial_labels['partially-shipped'] . '">' . $this->wc_partial_labels['partially-shipped'] . ' - ' . $shipped['item_qty'] . '</span></a>';
						} elseif (isset($shipped['item_qty']) && $shipped['item_qty'] > 0 && $shipped['item_qty'] == $item_data['quantity']) {
							$icon = '<a href="javascript:void(0);" class="wxp-top" title="' . $this->wc_partial_labels['shipped'] . ': ' . $shipped['item_qty'] . '/' . $item_data['quantity'] . '"><span class="wxp-shipped wxp-ship-status" title="' . $this->wc_partial_labels['shipped'] . '">' . $this->wc_partial_labels['shipped'] . ' - ' . $shipped['item_qty'] . '</span></a>';
						} elseif (isset($shipped['item_qty']) && $shipped['item_qty'] < 1) {
							$icon = '<a href="javascript:void(0);" class="wxp-top" title="' . $this->wc_partial_labels['not-shipped'] . '"><span class="wxp-not-shipped wxp-ship-status" title="' . $this->wc_partial_labels['not-shipped'] . '">' . $this->wc_partial_labels['not-shipped'] . ' - ' . $item_data['quantity'] . '</span></a>';
						} elseif ($item_id && ! array_key_exists($item_id, $wxp_shipments)) {
							$icon = '<a href="javascript:void(0);" class="wxp-top" title="' . $this->wc_partial_labels['not-shipped'] . '"><span class="wxp-not-shipped wxp-ship-status" title="' . $this->wc_partial_labels['not-shipped'] . '">' . $this->wc_partial_labels['not-shipped'] . ' - ' . $item_data['quantity'] . '</span></a>';
						}
					} elseif ($item_id && !array_key_exists($item_id, $wxp_shipments)) {
					...
					$icon = '<a href="javascript:void(0);" class="wxp-top" title="' . $this->wc_partial_labels['shipped'] . ': ' . $item_data['quantity'] . '/' . $item_data['quantity'] . '"><span class="wxp-shipped wxp-ship-status" title="' . $this->wc_partial_labels['shipped'] . '">' . $this->wc_partial_labels['shipped'] . ' - ' . $item_data['quantity'] . '</span></a>';
				}
			}
		}
	...
	function get_item_status($item_id, $item, $order)
	{
		...
		if (is_a($product, 'WC_Product') && !$product->is_virtual()) {
			if ($item_id && array_key_exists($item_id, $wxp_shipments)) {
				if (isset($shipped['item_qty']) && $shipped['item_qty'] > 0 && $shipped['item_qty'] < $item_data['quantity']) {
					$icon = __('Partially Shipped', 'wc-partial-shipment') . ' X ' . $shipped['item_qty'];
				} elseif (isset($shipped['item_qty']) && $shipped['item_qty'] > 0 && $shipped['item_qty'] == $item_data['quantity']) {
					$icon = __('Shipped', 'wc-partial-shipment') . ' X ' . $shipped['item_qty'];
				} elseif (isset($shipped['item_qty']) && $shipped['item_qty'] < 1) {
					$icon = __('Not Shipped', 'wc-partial-shipment') . ' X ' . $item_data['quantity'];
				} elseif ($item_id && ! array_key_exists($item_id, $wxp_shipments)) {
					$icon = __('Not Shipped', 'wc-partial-shipment') . ' X ' . $item_data['quantity'];
				}
			} elseif ($item_id && !array_key_exists($item_id, $wxp_shipments)) {
			...
			$icon = __('Shipped', 'wc-partial-shipment') . ' X ' . $item_data['quantity'];
		}
